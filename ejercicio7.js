...
ctx.beginPath();                                       // Inicia un nuevo trazado.
ctx.arc(240, 160, 20, 0, Math.PI * 2, false);         // Dibuja un círculo de radio 20.
...
function drawBall() {                                  // Función que dibuja la bola.
    ...
    ctx.fillStyle = "#0033FF";                         // Define el color azul de la bola.
...
var paddleX = (canvas.width - paddleWidth) / 2;        // Coloca la paleta en el centro horizontal.
...
function keyUpHandler(e) {                             // Función que detecta la tecla liberada.
...
function drawPaddle() {                                // Función que dibuja la paleta.
    ...
    ctx.fillStyle = "#0095DD";                         // Define el color azul de la paleta.
...
var brickRowCount = 3;                                 // Define la cantidad de filas de ladrillos.
var brickColumnCount = 5;                              // Define la cantidad de columnas de ladrillos.
var brickWidth = 75;                                   // Define el ancho de cada ladrillo.
var brickHeight = 20;                                  // Define la altura de cada ladrillo.
var brickPadding = 10;                                 // Define el espacio entre los ladrillos.
...
for (var c = 0; c < brickColumnCount; c++) {           // Recorre todas las columnas.
    bricks[c] = [];                                    // Crea un arreglo para cada columna.

    for (var r = 0; r < brickRowCount; r++) {          // Recorre todas las filas.
        bricks[c][r] = { x: 0, y: 0, status: 1 };      // Guarda posición y estado activo del ladrillo.
    }
}

function drawBricks() {                                // Función que dibuja los ladrillos activos.
    for (var c = 0; c < brickColumnCount; c++) {       // Recorre las columnas.
        for (var r = 0; r < brickRowCount; r++) {      // Recorre las filas.
            if (bricks[c][r].status == 1) {            // Comprueba si el ladrillo sigue activo.
                var brickX = c * (brickWidth + brickPadding) + brickOffsetLeft; // Calcula la posición horizontal.
                var brickY = r * (brickHeight + brickPadding) + brickOffsetTop; // Calcula la posición vertical.
...
function collisionDetection() {                        // Detecta las colisiones entre la bola y los ladrillos.
    for (var c = 0; c < brickColumnCount; c++) {       // Recorre las columnas.
        for (var r = 0; r < brickRowCount; r++) {      // Recorre las filas.
            var b = bricks[c][r];                      // Guarda el ladrillo actual en una variable.

            if (b.status == 1) {                       // Comprueba si el ladrillo sigue activo.
                if (
                    x > b.x &&                         // Comprueba el lado izquierdo del ladrillo.
                    x < b.x + brickWidth &&            // Comprueba el lado derecho del ladrillo.
                    y > b.y &&                         // Comprueba la parte superior del ladrillo.
                    y < b.y + brickHeight              // Comprueba la parte inferior del ladrillo.
                ) {
...
function draw() {                                      // Función principal de la animación.
    ...
    collisionDetection();                              // Comprueba las colisiones con los ladrillos.
    ...
    if (rightPressed && paddleX < canvas.width - paddleWidth) { // Comprueba el movimiento hacia la derecha.
        paddleX += 7;                                  // Mueve la paleta hacia la derecha.
    }
    else if (leftPressed && paddleX > 0) {             // Comprueba el movimiento hacia la izquierda.
