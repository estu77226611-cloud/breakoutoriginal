...
function draw() {                                      // Función principal de la animación.
    ...
    if (x + dx > canvas.width - ballRadius || x + dx < ballRadius) { // Verifica si la bola toca los bordes laterales.
        dx = -dx;                                      // Invierte la dirección horizontal.
    }

    if (y + dy < ballRadius) {                         // Verifica si la bola toca el borde superior.
        dy = -dy;                                      // Invierte la dirección vertical.
    }
    else if (y + dy > canvas.height - ballRadius) {   // Verifica si la bola llega al borde inferior.
        if (x > paddleX && x < paddleX + paddleWidth) { // Comprueba si la bola golpeó la paleta.
            dy = -dy;                                  // Hace rebotar la bola.
        }
        else {                                         // Si la bola no golpea la paleta.
            alert("GAME OVER");                        // Muestra el mensaje de fin del juego.
            document.location.reload();                // Reinicia la página para comenzar de nuevo.
        }
    }

    if (rightPressed && paddleX < canvas.width - paddleWidth) { // Mueve la paleta hacia la derecha.
        paddleX += 7;                                  // Desplaza la paleta 7 píxeles.
    }
    else if (leftPressed && paddleX > 0) {             // Mueve la paleta hacia la izquierda.
        paddleX -= 7;                                  // Desplaza la paleta 7 píxeles.
    }
