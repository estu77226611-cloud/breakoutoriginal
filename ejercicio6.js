...
var canvas = document.getElementById("myCanvas");      // Obtiene el canvas del archivo HTML.
var ctx = canvas.getContext("2d");                     // Permite dibujar elementos en dos dimensiones.
...
// El rebote de la bola se controla dentro de la función draw().
...
function keyDownHandler(e) {                           // Controla las teclas presionadas.
...
function keyUpHandler(e) {                             // Controla las teclas liberadas.
    if (e.keyCode == 39) {                             // Comprueba la flecha derecha.
...
// El rebote con la paleta y el mensaje GAME OVER se controlan en draw().
...
var brickOffsetTop = 30;                               // Define el espacio superior de los ladrillos.
var brickOffsetLeft = 30;                              // Define el espacio izquierdo de los ladrillos.

var bricks = [];                                       // Crea un arreglo para almacenar los ladrillos.

for (var c = 0; c < brickColumnCount; c++) {           // Recorre las columnas de ladrillos.
    bricks[c] = [];                                    // Crea un arreglo para cada columna.

    for (var r = 0; r < brickRowCount; r++) {          // Recorre las filas de ladrillos.
        bricks[c][r] = { x: 0, y: 0 };                 // Guarda la posición inicial de cada ladrillo.
    }
}
...
function drawBricks() {                                // Función que dibuja todos los ladrillos.
    for (var c = 0; c < brickColumnCount; c++) {       // Recorre las columnas.
        for (var r = 0; r < brickRowCount; r++) {      // Recorre las filas.
            ...
            bricks[c][r].x = brickX;                   // Guarda la posición horizontal del ladrillo.
            bricks[c][r].y = brickY;                   // Guarda la posición vertical del ladrillo.
...
function draw() {                                      // Función principal de la animación.
    ...
    drawBricks();                                      // Dibuja los ladrillos.
    ...
    if (y + dy < ballRadius) {                         // Comprueba el borde superior.
        ...
        else {                                         // Se ejecuta si la bola no toca la paleta.
